...
| `architecture/specs/shared-context-pack.md` | Canonical shared-context spec needed explicit PDSL contracts for controller lifecycle, prompt-context views, validation, and consumer rules, plus a precise base contract for project-origin prompt assets. | Added operational PDSL units that formalize session-pack reuse, fail-closed validation, and prompt-consuming agent invariants; clarified that controllers whose model includes project prompt surfaces must support `origin = "project"` end-to-end through pack selection and dispatch. | Broader example and migration-guidance sections remain prose-first reference content. |
| `architecture/specs/sysprompts.md` | Project sysprompt spec still described loading/validation behavior in prose and did not explicitly state the shared-context-pack handoff model or `project`-origin recording requirement. | Added PDSL units for prompt-asset classification, controller-owned loading, and warning/error handling; updated the loading algorithm prose to publish prompt text into `SHARED_CONTEXT_PACK` as `origin = "project"` assets, then deliver it through `prompt_context_view`. | Example project files remain illustrative prose/examples. |
...
- This phase intentionally stayed within the prompt-bearing files that still mixed active runtime behavior with prose-only control flow or stale shared-context assumptions, including the contract gap around project-origin prompt assets.
