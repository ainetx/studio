...
- Search across `skills/studio/agents.toml`, `skills/studio/agents/`, and `workflows/` found zero occurrences of `prompt_context_requirements` and zero occurrences of `prompt_context_view`.
...
| --- | --- | --- |
| No prompt-context declarations anywhere in registered agent metadata | `rg -n 'prompt_context_requirements|prompt_context_view' skills/studio/agents.toml skills/studio/agents workflows` returned no matches. | Phase 4 will need to add prompt-context declarations rather than only cleaning up direct-load prose. |
