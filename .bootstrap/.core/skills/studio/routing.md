...
Note: `/cf-studio` is an alias for `/cf` — both invoke the same Constructor Studio skill.

Direct CLI commands still require write confirmation when write-capable.
Use `{cfs_cmd} --json agents --agent <name>` for agent lookup. Run `init`,
`delegate`, and `update` without `--json`.

Workflow routing priority:
1. `delegate` -> open and follow `{cf-studio-path}/.core/skills/studio/agents/cf-ralphex.md`.
2. `compile phase` -> open and follow `{cf-studio-path}/.core/skills/studio/agents/cf-phase-compiler.md`.
3. `execute phase` -> open and follow `{cf-studio-path}/.core/skills/studio/agents/cf-phase-runner.md`.
...
8. map / dependency map / cfs map / visualize dependencies / render graph ->
   open and follow `workflows/cf-map.md`.
9. auto-config / configure project / scan brownfield / generate rules ->
   open and follow `workflows/auto-config.md`.
10. migrate from cypilot / migrate-from-cypilot -> open and follow `migrate-from-cypilot.md`.
...
Oversized raw inputs over `500` lines (line count of the user's pasted text or directly provided files) must surface the raw-input-overflow rule (`{cf-studio-path}/.core/requirements/raw-input-overflow.md`) which offers `/cf-plan` before direct generate/analyze continues. This raw-input threshold is distinct from the workflow-level estimated-context gates (`analyze`: > 2000 lines, `generate`: > 2500 lines); do not collapse the three thresholds.
