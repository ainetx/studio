...
| 26 | Generic 6 nav slots ("next", "tell me more", "anything else", "summary", "any other questions", "stop") | Slots must be concrete and contextual; Next / Deeper / Lateral / Recap / Ask / Wrap each carry specific semantics. Slot 7 (`Send comments — preview`) is the only conditional slot and is added per AP-#40 / phases.md. |
...
| 40 | Auto-dispatching the queue on bare `send comments` without preview, OR hiding the `send comments` action entirely from the nav block | Bare `send comments` (or slot 7) MUST render preview only. Dispatch requires `send comments now` / `flush queue` / `route queued` or `Y` in the preview prompt. The nav block MUST surface a 7th slot `7. Send comments — preview (K queued)` whenever the queue has ≥ 1 active item; the slot is omitted only when the queue is empty. Fat-finger protection is at the dispatch boundary (Y / `send comments now`), NOT at the preview boundary — hiding preview from the menu loses visibility without adding safety. |
...
- [ ] Artifact disposition (`chat-only` / `save-to-file` / `post-to-resource` / `mixed`) resolved at session start via the always-ask prompt — emitted after mode resolution, before role/audience confirmation, with the resolved post-to-resource branch shown in option 3's label (PR posting via {handler} / Notion-Jira-GitLab posting via {handler} / local-file generate-routing dispatch of `cf-constructor-generate` / unavailable falling back to save-to-file). The project `artifact_disposition` preference informed the suggested default but did NOT bypass the prompt. `state.post_to_resource_branch` is recorded when `selected_tag == "post-to-resource"`
