...
| 35 | Offering the cfc-routing sub-prompt when `handle.local_editable == false` or `handle.cfc_route_available == false` | Both flags AND-gate sub-prompt visibility. When either is false, methodology MUST suppress the sub-prompt entirely (no flicker, no "unavailable" message). The closed enum `local_editable_reason` exists for telemetry, not for surfacing failed offers. See `storytelling-preflight.md` § Step 5b. |
| 36 | Auto-dispatching cfc-route on the suggested default without an explicit user reply | The cfc-routing sub-prompt is a hard interaction boundary — methodology MUST end the turn after emitting and wait for a concrete numeric choice (1/2/3/4). Auto-dispatch on the suggested-default heuristic bypasses consent-per-comment and silently mutates files. See `storytelling-gate.md` § Gate: cfc-routing. |
| 37 | Pausing the storytelling thread on a cfc-routing dispatch (waiting synchronously for cfc completion before emitting the next portion) | Dispatch is fire-and-forget. The orchestrator emits a same-turn ack (`🚀 routed Q-N to cfc — session-id …`) before the next portion's nav block, then continues navigation. Results arrive in their own turns with `[cfc-route Q-N]:` prefix and MUST NOT merge into a portion body. Synchronous blocking defeats interactive cadence. |
...
| 40 | Auto-dispatching the queue on bare `send comments` without preview | Bare `send comments` (or any intent-equivalent) MUST render preview only. Dispatch requires either `send comments now` / `flush queue` / `route queued` or a `Y` reply to the preview's structured batch prompt. Eliminates fat-finger destructive flushes. |
...
| 53 | Pasting the hot body inside a Mode C paste-handoff block | Mode C is bounded by chat-readability (~3 KB). The paste block MUST contain only: 1-line intent banner, breadcrumb + anchor pointer, the comment quote (≤ 800 B), and the `cfc` invocation line. The hot body is referenced by `pack_handle` / file:line — never pasted. Users can re-open the file faster than reading pasted code. |
...
| 63 | Placing a failure notice inside a portion's Opening / Body / Mode-lens / Diagram / Source refs / viz-marker / progress marker / Nav | Failure notices land BETWEEN portions, in the methodology's next turn, BEFORE the next Opening, separated by a single blank line. The 8-element portion shape is preserved. No `[cfc-route]` fenced blocks. No `---` separators. |
| 64 | Pasting the full cfc validator output (multi-page structured JSON) into chat | `validation_rejected` notices show ONLY the first sentence head (truncated with `…` to fit a single 80-col terminal line) plus `(+N-1 more in dispatch-failures.jsonl)` overflow indicator. Full payload (all issues, rule context, draft path, suggested-edit verb) is appended to the dispatch-failures NDJSON; chat shows the head only. |
