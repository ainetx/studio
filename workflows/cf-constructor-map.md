...
version: 1.0
purpose: Guide cfc map workflow from pre-flight through validation
---

# Cyber Constructor Map Workflow
...
| User intent | Route |
|---|---|
| Generate map of current project | `generate.md` → `cf-constructor-map.md` |
...
| Step | Action |
|---|---|
| Check artifacts.toml | `python3 {cf-constructor-path}/.core/skills/cypilot/scripts/cypilot.py --json info` (look for `[[systems.codebase]]` entries) |
| Check workspace config | Look for `.cf-constructor-workspace.toml` or `.cypilot-workspace.toml` in project root |
...
- `include-workspace` → scan workspace sources (requires .cf-constructor-workspace.toml or .cypilot-workspace.toml).
...
## Phase 3: Generate
**Goal**: invoke `cfc map` and produce output.

| Scope decision | Command |
|---|---|
| Local markdown only | `python3 {cf-constructor-path}/.core/skills/cypilot/scripts/cypilot.py --json map --no-source [--out PATH] [--format html\|json]` |
| Include workspace sources | `python3 {cf-constructor-path}/.core/skills/cypilot/scripts/cypilot.py --json map [--out PATH] [--format html\|json]` |
| Local + source code only | `python3 {cf-constructor-path}/.core/skills/cypilot/scripts/cypilot.py --json map --local-only [--out PATH] [--format html\|json]` |
...
| Route to diagnostics | If dangling cpts found, suggest `cfc where-used <cpt-id>` or `cfc list-ids` for cross-repo IDs |

## Quick Reference

| Intent | Flags |
|---|---|
| Markdown-only quick map | `cfc map --no-source` |
| Single-repo map (skip federation) | `cfc map --local-only` |
| Machine-readable graph | `cfc map --format json --out map.json` |
| Custom categories | `cfc map --config md-map.toml` |
| Self-contained HTML (no sidecar) | `cfc map --inline-data` |
| Debug layout | `cfc map -v` or `cfc map --verbose` |
...
- For dangling cpts, use `cfc where-used <cpt-id>` to find missing definitions
...
3. Check for dangling cpts — Run cfc where-used <cpt-id> to diagnose phantom references.
