...
This template routes the next agent to `/cf-constructor-analyze` (IMMEDIATE review) and asks for
findings with severity, evidence, risks, regressions, and recommended fixes. The Plan-Review
template routes to `/cf-constructor-plan` (PHASED review plan) instead. The two diverge across
