...
- [3. Processes / Business Logic (CDSL)](#3-processes--business-logic-cdsl)
  ...
  - [Build cpt Edges Algorithm](#build-cpt-edges-algorithm)
...
**Steps**:
1. [x] - `p1` - User invokes `cfc map [--out PATH] [--format html|json] [--config FILE] [--no-source] [--local-only] [--inline-data] [--verbose]`
...
4. [x] - `p1` - **FOR EACH** reachable source, invoke `scan_repo` using `cpt-cypilot-algo-map-scan:p1`
5. [x] - `p1` - Categorize all nodes using `cpt-cypilot-algo-map-categorize:p1`
6. [x] - `p1` - Extract file-link edges using `cpt-cypilot-algo-map-file-links:p1`
7. [x] - `p1` - Build cpt edges and phantom nodes using `cpt-cypilot-algo-map-cpt-edges:p1`
8. [x] - `p1` - Enrich edges with definition context using `cpt-cypilot-algo-map-enrich:p1`
9. [x] - `p1` - Compute layout using `cpt-cypilot-algo-map-layout:p1`
10. [x] - `p1` - Serialize to JSON using `cpt-cypilot-algo-map-render-json:p1`
11. [x] - `p1` - **IF** format is html, render HTML using `cpt-cypilot-algo-map-render-html:p1`; else write JSON directly
12. [x] - `p1` - Print summary: node counts, edge counts, phantom count, output path
## 3. Processes / Business Logic (CDSL)

### Scan Project Files

- [x] `p1` - **ID**: `cpt-cypilot-algo-map-scan`
...
**Steps**:
1. [x] - `p1` - Walk all `.md` files under project root (both root-level and recursive) using `cpt-cypilot-algo-map-scan-walker:p1`
...
3. [x] - `p1` - **IF** `no_source` is False, load `artifacts.toml` and enumerate `[[systems.codebase]]` entries using `cpt-cypilot-algo-map-scan-walker:p1`
...
- [x] `p1` - **ID**: `cpt-cypilot-algo-map-categorize`
...
2. [x] - `p1` - **FOR EACH** node, apply three-step priority chain using `cpt-cypilot-algo-map-categorize-chain:p1`
3. [x] - `p1` - Phantom nodes receive `category = "_undefined"` and `category_origin = "phantom"` unconditionally
### Build cpt Edges

- [x] `p1` - **ID**: `cpt-cypilot-algo-map-cpt-edges`
...
2. [x] - `p1` - **FOR EACH** non-phantom node, iterate `cpt_uses` skipping `md-def` entries using `cpt-cypilot-algo-map-build-cpt-edges:p1`
...
- [x] `p1` - **ID**: `cpt-cypilot-algo-map-file-links`
...
3. [x] - `p1` - Resolve each target link using `cpt-cypilot-algo-map-resolve-link:p1`; skip external URLs, self-links, duplicates
4. [x] - `p1` - Emit `file-link` edge for each resolved target found in the known node index
5. [x] - `p1` - **RETURN** list of file-link edges
### Enrich Edges

- [x] `p1` - **ID**: `cpt-cypilot-algo-map-enrich`
...
2. [x] - `p1` - For each non-dangling cpt edge, resolve the target markdown file path from its source root
...
- [x] `p1` - **ID**: `cpt-cypilot-algo-map-layout`
...
- [x] `p1` - **ID**: `cpt-cypilot-algo-map-render-json`
...
- [x] `p1` - **ID**: `cpt-cypilot-algo-map-render-html`
...
- [x] `p1` - **ID**: `cpt-cypilot-algo-map-data-model`
...
**Implements**:
- `cpt-cypilot-algo-map-scan:p1`
- `cpt-cypilot-algo-map-cpt-edges:p1`
- `cpt-cypilot-algo-map-file-links:p1`
...
- `cpt-cypilot-algo-map-render-json:p1`
...
**Implements**:
- `cpt-cypilot-algo-map-render-html:p1`
- `cpt-cypilot-algo-map-layout:p1`
