...
class TestTrackInvocation(unittest.TestCase):
    ...
    def test_disabled_by_env(self):
        """When CFS_TELEMETRY=0, no thread is spawned."""
        with patch.dict(os.environ, {"CFS_TELEMETRY": "0"}):
...
class TestRotateLogs(unittest.TestCase):
    ...
    def test_deletes_old_files(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            with patch("studio_proxy.telemetry.LOG_DIR", log_dir):
                with patch.dict(os.environ, {"CFS_TELEMETRY_RETENTION_DAYS": "5"}):
                    _rotate_logs()
    ...
    def test_invalid_retention_days_uses_default(self):
        """Invalid CFS_TELEMETRY_RETENTION_DAYS falls back to default."""
        with TemporaryDirectory() as tmpdir:
            log_dir = Path(tmpdir) / "logs"
            log_dir.mkdir()
            old_file = log_dir / "2020-01-01.log"
            ...
            os.utime(old_file, (0, 0))
            with patch("studio_proxy.telemetry.LOG_DIR", log_dir):
                with patch.dict(os.environ, {"CFS_TELEMETRY_RETENTION_DAYS": "not_a_number"}):
                    _rotate_logs()
            self.assertFalse(old_file.exists())
    ...
    def test_zero_retention_clamped_to_one(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            with patch("studio_proxy.telemetry.LOG_DIR", log_dir):
                with patch.dict(os.environ, {"CFS_TELEMETRY_RETENTION_DAYS": "0"}):
                    _rotate_logs()
...
class TestBuildOtlpLogs(unittest.TestCase):
    ...
    def test_structure(self):
        ...
        self.assertEqual(attrs["service.name"], "constructor-studio")
...
class TestTelemetryWorkerIntegration(unittest.TestCase):
    ...
    def test_full_flow(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            with patch("studio_proxy.telemetry.LOG_DIR", log_dir), \
                 ...
                 patch.dict(os.environ, {"CFS_TELEMETRY_URL": "http://localhost:4318/v1/logs"}), \
                ...
                self.assertEqual(data["constructor_studio_version"], "3.5.1-test")
                mock_urlopen.assert_called_once()

    def test_no_http_when_url_not_set(self):
        """Worker should skip HTTP when CFS_TELEMETRY_URL is not set."""
        with TemporaryDirectory() as tmpdir:
            log_dir = Path(tmpdir) / "logs"
            mock_git = MagicMock()
            mock_git.stdout = "user.name Test\nuser.email test@test.com"
            env = os.environ.copy()
            env.pop("CFS_TELEMETRY_URL", None)
            with patch("studio_proxy.telemetry.LOG_DIR", log_dir), \
                 patch("studio_proxy.telemetry.subprocess.run", return_value=mock_git), \
    ...
    def test_rejects_invalid_url_scheme(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            with patch("studio_proxy.telemetry.LOG_DIR", log_dir), \
                 patch("studio_proxy.telemetry.subprocess.run", return_value=mock_git), \
                 patch.dict(os.environ, {"CFS_TELEMETRY_URL": "file:///etc/passwd"}), \
                 patch("studio_proxy.telemetry.urlopen") as mock_urlopen, \
                 patch("studio_proxy.__version__", "3.5.1-test"):
