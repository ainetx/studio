...
- **TOML key rewrites** (`cypilot_path` → `cf-constructor-path`): if the file is a TOML file or contains TOML in a fenced Markdown block, prefer key-level substitution (replace the whole line `cypilot_path = "..."` with `cf-constructor-path = "..."` preserving the value). Falls back to substring substitution if the line layout is unusual.
- **URL rewrites** (`github.com/cyberfabric/cyber-pilot` → `cyber-constructor`): apply as substring; the URL form is well-defined.
- **Proper-noun rewrites** (`Cypilot` → `Cyber Constructor`): apply as substring with `replace_all=true` per file. The result text is longer; verify nothing breaks (e.g. table column alignment, code-block indentation).
- **Command-form rewrites** (`` `cpt ` `` → `` `cfc ` `` AND ` cpt ` → ` cfc `): apply BOTH substring forms in order (backtick form first, then space-padded). Do NOT rewrite anything that doesn't match the well-defined patterns. Per `project_markdown_rewriter_conservative.md`, `cpt.` / line-start `cpt` / `cpt!` etc. stay as-is (those are needs-review B-items).
- **Kit slug rewrite** (`cypilot-sdlc` → `sdlc`): apply only in TOML / YAML / JSON files where this slug appears in a `kit = ...` / `slug = ...` context. If it appears in code (variable name, etc.) treat as B (skip in this step).
...
1. Workspace file rename:
     mv .cypilot-workspace.toml .cf-constructor-workspace.toml
...
2. Cascade migration into workspace member `{name}` at `{path}`:
     ...
     # After it lands, re-run this skill (cf-constructor migrate from cypilot)
