...
The **registered native sub-agent set** is the set of `cf-constructor-*` sub-agents the host has loaded into its dispatch tool list for the current chat session. The orchestrator determines membership by:

1. **Announced-list method (preferred):** when the host announces its tool list at session start (Claude Code's tool registry, OpenAI Assistant tools field, etc.), the orchestrator inspects that list and treats any `cf-constructor-*` name present in it as registered.
