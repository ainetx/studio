"""
Cyber Pilot -> Cyber Constructor migration command.

This command is intentionally small and mechanical: it copies the legacy
Cypilot install directory to the Cyber Constructor directory, rewrites the
root AGENTS/CLAUDE managed blocks, updates kit sources, then delegates normal
core and kit refresh to ``cfc update``.
"""

# @cpt-flow:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1
# @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-migration-module
import contextlib
import io
import json
import os
...
CONSTRUCTOR_KIT_SOURCE = "github:cyberfabric/cyber-constructor-kit-sdlc"
...
# @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-migration-module
...
def migrate_from_cypilot(
    *,
    ...
    """Migrate a Cyber Pilot project in *project_root* to Cyber Constructor.
    ...
    """
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-resolve-dirs
    ...
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-resolve-dirs

    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-validate-dirs
    if legacy_error:
        return 1, {
            "status": "ERROR",
    ...
    backups: List[str] = []
    warnings: List[str] = []
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-validate-dirs

    if not dry_run and not force_overwrite_root:
        # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-probe-root-dirty
        ...
        # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-probe-root-dirty

    if legacy_dir != target_dir:
        if target_dir.exists():
            # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-target-exists
            if not force:
                return 1, {
                    "status": "ERROR",
            ...
            # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-target-exists
            if not dry_run:
                # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-replace-target
                ...
                if backup_path is None:
                    return 1, {
                        ...
                        "project_root": project_root.as_posix(),
                        "from_dir": legacy_rel,
                        "cf_constructor_dir": target_dir.as_posix(),
                ...
                # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-replace-target
            else:
                # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-replace-target
                actions["target_dir"] = "replaced"
                # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-replace-target
        else:
            # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-create-target
            ...
            if not dry_run:
                try:
                    ...
                    return 1, error_result
            # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-create-target
    else:
        # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-reuse-target
        actions["target_dir"] = "reused"
        # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-reuse-target

    if not dry_run:
        # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-post-copy-rewrites
        ...
        # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-post-copy-rewrites
    else:
        # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-dry-run-actions
        ...
        # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-dry-run-actions

    update_rc: Optional[int] = None
    update_result: Optional[Any] = None
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-followup-update
    ...
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-followup-update

    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-return-result
    result: Dict[str, Any] = {
    ...
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-return-result
...
def detect_legacy_cypilot_install(project_root: Path) -> Optional[str]:
    ...
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-detect-legacy
    return _read_legacy_install_dir(project_root)
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-detect-legacy


def resolve_cypilot_project_root(project_root_arg: Optional[str]) -> Optional[Path]:
    """Resolve a project root that may contain either Cyber Constructor or Cyber Pilot markers."""
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-resolve-project-root
    return _resolve_project_root(project_root_arg)
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-resolve-project-root
...
def should_migrate_from_cypilot(
    ...
    *,
    ...
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-should-migrate
    ...
    return False
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-should-migrate


def migration_declined_result(project_root: Path, legacy_rel: str, *, dry_run: bool = False) -> Dict[str, Any]:
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-declined-result
    return {
        "status": "ABORTED",
        "message": "Cyber Pilot migration declined; Cyber Constructor not initialized.",
        "project_root": project_root.as_posix(),
        "from_dir": legacy_rel,
        "dry_run": bool(dry_run),
        "actions": {"migration": "declined"},
    }
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-declined-result
...
def ensure_supported_legacy_version(
    *,
    ...
    """
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-check-legacy-version
    ...
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-check-legacy-version

    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-preflight-dry-run
    ...
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-preflight-dry-run

    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-prompt-legacy-update
    ...
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-prompt-legacy-update

    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-run-legacy-update
    ...
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-run-legacy-update

    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-check-updated-version
    ...
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-check-updated-version


def merge_legacy_preflight_result(result: Dict[str, Any], preflight: Dict[str, Any]) -> Dict[str, Any]:
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-merge-preflight
    ...
    return result
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-merge-preflight


def read_legacy_cypilot_version(legacy_dir: Path) -> Optional[str]:
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-read-version
    for init_file in (
        ...
        if version:
            return version
    return None
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-read-version
...
def _normalize_legacy_version(version: Optional[str]) -> Optional[str]:
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-normalize-version
    if not version:
        return None
    value = version.strip()
    if value.startswith("v"):
        value = value[1:]
    return value
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-normalize-version


def _prompt_update_legacy_cypilot(project_root: Path, legacy_rel: str, version: Optional[str]) -> bool:
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-prompt-update-ui
    sys.stderr.write("\n")
    sys.stderr.write(
    ...
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-prompt-update-ui
...
def _run_legacy_update_to_baseline(project_root: Path) -> Dict[str, Any]:
    ...
    sys.stderr.write(
        ...
        "may be in a partial state. Re-run cfc init or cfc update to retry.\n"
    )
    try:
        proc = subprocess.run(
            cmd,
        ...
        sys.stderr.write(
            ...
            "in a partial state; re-run cfc init or cfc update to retry.\n"
        )
        return {
            "status": "ERROR",
            "command": cmd,
...
def _prompt_migrate_from_cypilot(
    ...
    *,
    ...
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-prompt-migration-ui
    sys.stderr.write("\n")
    ...
    sys.stderr.write(f"  {prompt or 'Migrate it to Cyber Constructor now? [y/N] '}")
    sys.stderr.flush()
    try:
        answer = input().strip().lower()
    except (EOFError, KeyboardInterrupt):
        answer = ""
    return answer in ("y", "yes")
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-prompt-migration-ui


def _run_followup_update(project_root: Path, *, yes: bool) -> tuple[int, Optional[Any]]:
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-run-followup-update
    ...
    try:
        ...
        return rc, raw
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-run-followup-update


def _restore_target_backup_after_replace_failure(backup_path: Path, target_dir: Path) -> tuple[str, Optional[str]]:
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-restore-target
    try:
        if target_dir.exists():
            shutil.rmtree(target_dir)
    ...
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-restore-target
...
def _run_post_copy_rewrites(
    *,
    ...
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-backup-root-files
    ...
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-backup-root-files

    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-run-rewrite-steps
    ...
    for rewrite_step, rewrite in rewrites:
        try:
            ...
            return result
    return None
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-run-rewrite-steps
...
def _resolve_project_child_dir(project_root: Path, rel_path: str, option_name: str) -> tuple[Path, Optional[str]]:
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-child-dir-guard
    ...
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-child-dir-guard


def _read_legacy_install_dir(project_root: Path) -> Optional[str]:
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-read-legacy-install
    content = ""
    ...
    if agents_file.is_file():
        ...
        for key in ("cypilot_path", "cypilot"):
            ...
            if isinstance(value, str) and value.strip():
                return value.strip()
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-read-legacy-install
...
def _probe_root_files_dirty(project_root: Path, paths: List[Path]) -> List[str]:
    ...
    """
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-probe-git-dirty
    ...
    return dirty
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-probe-git-dirty
...
def _replace_root_block_with_warnings(target_file: Path, install_dir: str, warnings: List[str]) -> str:
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-replace-root-blocks
    ...
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-replace-root-blocks
...
def _remove_legacy_block(content: str) -> tuple[str, bool]:
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-remove-legacy-block
    ...
    changed = False
    malformed = False
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-remove-legacy-block
...
def _migrate_core_toml(core_toml: Path, warnings: Optional[List[str]] = None) -> str:
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-migrate-core-toml
    ...
    if isinstance(kits, dict):
        ...
        for kit_data in kits.values():
            ...
            if kit_data.get("source") == LEGACY_KIT_SOURCE:
                ...
                changed = True
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-migrate-core-toml
...
def _migrate_artifacts_toml(
    ...
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-migrate-artifacts-toml
    ...
    if changed:
        toml_utils.dump(data, artifacts_toml, header_comment="Cyber Constructor artifacts registry")
        return "updated"
    return "unchanged"
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-migrate-artifacts-toml
...
def _migrate_config_markdown(config_dir: Path) -> List[str]:
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-migrate-config-markdown
    changed: List[str] = []
    for name in ("AGENTS.md", "SKILL.md", "README.md"):
        ...
        new_text = (
            text
            .replace("{cypilot_path}", "{cf-constructor-path}")
            .replace("`cpt ", "`cfc ")
            .replace(" cpt ", " cfc ")
            .replace("Cypilot", "Cyber Constructor")
        )
        if new_text != text:
            path.write_text(new_text, encoding="utf-8")
            changed.append(name)
    return changed
    # @cpt-end:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-migrate-config-markdown


def _human_migrate_ok(data: Dict[str, Any]) -> None:  # pyright: ignore
    # @cpt-begin:cpt-cypilot-flow-core-infra-migrate-from-cypilot:p1:inst-human-output
    prefix = "[dry-run] " if data.get("dry_run") else ""
    ui.header(f"{prefix}Cyber Pilot -> Cyber Constructor Migration")
    ui.detail("Project root", str(data.get("project_root", "")))
    ui.detail("From", str(data.get("from_dir", "")))
    ui.detail("To", str(data.get("cf_constructor_dir", "")))
