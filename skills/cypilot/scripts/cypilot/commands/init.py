...
CACHE_DIR = Path.home() / ".cf-constructor" / "cache"
CORE_SUBDIR = ".core"
GEN_SUBDIR = ".gen"
DEFAULT_INSTALL_DIR = ".cf-constructor"
...
def _copy_from_cache(cache_dir: Path, target_dir: Path, force: bool = False) -> Dict[str, str]:
    ...
    files remain from previous versions. This is the mode used by `cfc update`.
    ...
    # This is the mode used by `cfc update` which always passes force=True
...
def _core_readme() -> str:
    ...
    return (
        "# .core — Cyber Constructor Core Files\n"
        "\n"
        "**Do NOT edit files in this directory.**\n"
        "\n"
        "These files are copied from the Cyber Constructor cache (`~/.cf-constructor/cache/`) during\n"
        "`cfc init` or `cfc kit install`. They are the read-only reference copies of:\n"
        "\n"
        "- `skills/` — Cyber Constructor skill scripts and CLI entry points\n"
        "- `workflows/` — workflow definitions\n"
        "- `requirements/` — validation requirements\n"
        "- `schemas/` — JSON schemas for configuration files\n"
        "- `architecture/specs/` — traceability, CDSL, CLI, and kit specifications\n"
        "\n"
        "To update these files, run `cfc init --force` or `cfc kit update`.\n"
...
def _gen_readme() -> str:
    ...
    return (
        ...
        "**Do NOT edit files in this directory.**\n"
        "\n"
        "These files are auto-generated by Cyber Constructor during\n"
        "`cfc init`, `cfc kit install`, or `cfc update`.\n"
        "\n"
        "Contents:\n"
        "\n"
        ...
        "To update: `cfc update` or `cfc kit update`.\n"
...
def _config_readme() -> str:
    ...
    return (
        ...
        "  These are updated via `cfc update` or `cfc kit update`.\n"
        "\n"
        "## Tips\n"
        "\n"
        ...
        "- Kit files can be edited directly; `cfc kit update` shows a diff for changes.\n"
    )
...
def _read_existing_install(project_root: Path) -> Optional[str]:
    """
    Check if project already has Cyber Constructor installed by reading AGENTS.md TOML block.
...
def _compute_managed_block(install_dir: str) -> str:
    ...
    return (
        f"{MARKER_START}\n"
        f"```toml\n"
        f'cf-constructor-path = "{install_dir}"\n'
        f"```\n"
        f"{MARKER_END}"
    )
...
_DEFAULT_KIT_SOURCE = "cyberfabric/cyber-constructor-kit-sdlc"
...
def _prompt_kit_install_flag(interactive: bool) -> bool:
    ...
    if interactive and sys.stdin.isatty():
        ...
        sys.stderr.write("  This adds the default Cyber Constructor SDLC templates, workflows, and rules for typical project setup.\n")
...
def cmd_init(argv: List[str]) -> int:
    ...
    if interactive:
        sys.stderr.write("\n")
        sys.stderr.write("  \033[1mWelcome to Cyber Constructor\033[0m\n")
        sys.stderr.write("  Set up AI-powered architecture traceability for your project.\n")
        sys.stderr.write("  Cyber Constructor will create a configuration directory with design artifacts,\n")
    ...
    if existing_install_rel is not None and not args.force:
        ui.result(
            {
                "status": "FAIL",
                "message": "Cyber Constructor already initialized. Use 'cfc update' to upgrade or --force to reinitialize.",
                "project_root": project_root.as_posix(),
                "cypilot_dir": (project_root / existing_install_rel).as_posix(),
            },
            human_fn=lambda d: (
                ui.error("Cyber Constructor is already initialized in this project."),
                ui.detail("Directory", (project_root / existing_install_rel).as_posix()),
                ui.blank(),
                ui.hint("To refresh to the latest version:  cfc update"),
                ui.hint("To reinitialize from scratch:      cfc init --force"),
                ui.blank(),
            ),
        )
        return 2
    ...
    if args.install_dir is None and interactive:
        sys.stderr.write("\n")
        sys.stderr.write("  \033[2mCyber Constructor stores its files in a subdirectory of your project.\033[0m\n")
        sys.stderr.write("  \033[2mThis directory will contain .core/, .gen/, and config/ folders.\033[0m\n")
        install_rel = _prompt_path("Cyber Constructor directory (relative to project root)?", default_install_dir)
    else:
    ...
    if not CACHE_DIR.is_dir():
        ui.result(
            {
                "status": "ERROR",
                "message": f"Cyber Constructor cache not found at {CACHE_DIR}. Run 'cfc update' first.",
                "project_root": project_root.as_posix(),
            },
            human_fn=lambda d: (
                ...
                ui.blank(),
                ui.hint("Install Cyber Constructor first:  pipx install git+https://github.com/cyberfabric/cyber-constructor.git && cfc update"),
                ui.blank(),
            ),
        )
        return 1
    ...
    if core_toml_existed and not args.force:
        ...
        if not args.dry_run:
            toml_utils.dump(desired_core, core_toml_path, header_comment="Cyber Constructor project configuration")
    ...
    if config_agents_existed and not args.force:
        ...
        if not config_agents_existed and not args.dry_run:
            config_agents_path.write_text(
                ...
                "These rules are loaded alongside the generated rules in `{cf-constructor-path}/.gen/" + _AGENTS_FILENAME + "`.\n",
                encoding="utf-8",
            )
    ...
    if not args.dry_run:
        ...
        # @cpt-begin:cpt-cypilot-flow-core-infra-project-init:p1:inst-skip-kit-declined
            ...
            ui.info(f"Skipped kit installation. Install later: cfc kit install {_DEFAULT_KIT_SOURCE}")
    ...
    if registry_existed_before and not args.force:
        ...
        if not args.dry_run:
            toml_utils.dump(desired_registry, registry_path, header_comment="Cyber Constructor artifacts registry")
...
def _human_init_ok(
    ...
    ui.header(f"{prefix}Cyber Constructor Init")
    ...
    if dry:
        ...
        ui.success("Cyber Constructor initialized!")
        ui.blank()
        ui.info("Next steps:")
        ui.hint("1. Set up your IDE:  cfc generate-agents")
        ui.hint("2. Review config:    open " + install_rel + "/config/core.toml")
        ui.hint("3. Start using:      type '/cf-constructor' in your IDE chat")
    ui.blank()
